...
## Here we use backward stepwise model selection starting with the most complex model we think is reasonable.
# Model selection is carried out here by comparing AIC
# (we could also use BIC or log likelihood ratio tests - they achieve the same thing)

mod1<- lm(boldness~ mass+ diet + mass:diet, data= behave_data)

mod2<- lm(boldness~ mass+ diet, data= behave_data)

AIC(mod1, mod2)
summary(mod2)

mod3<- lm(boldness ~ mass, data= behave_data)
AIC(mod2, mod3)


null.lm<- lm(boldness~ 1, data= behave_data)
AIC(mod3, null.lm)
...
### We can describe the relationship suggested by the model simply by looking at the summary table
...
diet <- as.factor(rep(c('hi-fibre', 'hi-starch'), length.out=samp))
dat2<- data.frame('mass' = masses, 'diet'= diet) # it is stored in the "dat" data frame

## The following code develops the modeled relationships between the explanatory variables and
# the behavioral response variable , "feed.behav," which is a timed (in ms) behavioral response to the stimulus
...
mod1<- lm(feed.behav~ mass+ diet  + mass:diet, data= dat2)

mod2<- lm(feed.behav~ mass+ diet, data= dat2)
AIC(mod1, mod2)

summary(mod2)

mod3<- lm(feed.behav~ diet, data= dat2)
AIC(mod2, mod3)


mod.Null<- lm(feed.behav~ 1, data= dat2)
